...
const getFetch = new Promise((resolve, reject) => {
        let condition = true
        condition ? setTimeout(() => { resolve(Data) }, 1000) : reject(console.log("error en base de datos"))
    })  

const getItem = (id) => {
    return new Promise((resolve, reject) => {
        setTimeout(() => resolve(Data.find(prod => prod.id === id)), 1000)
    })
}

const getCategory = (categoryId) => {
    return new Promise((resolve, reject) => {
        setTimeout(() => resolve(Data.filter(prod => prod.categoria === categoryId)), 1000)
    })
}
export { getFetch }
export { getItem }
